...
static void encode_and_send(const int16_t *samples) {
    uint8_t pcma[MAX_FRAME_SZ];
    for (int i = 0; i < frame_sampc; i++)
...
static void tx_tick(void *arg) {
    ...
    if (now - (uint64_t)ptime > tx_next)
        tx_next = now + (uint64_t)ptime;
    else
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        case 't':
            ptime = atoi(optarg);
            if (ptime < 5 || ptime > MAX_PTIME) {
                re_fprintf(stderr, "Invalid ptime %s (5-%u)\n",
                           optarg, MAX_PTIME);
                return 1;
            }
            frame_sampc = SRATE * ptime / 1000;
            frame_bytes = frame_sampc * 2;
            break;
        case 'i':
            input_path = optarg;
            break;
